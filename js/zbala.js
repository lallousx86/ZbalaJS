...
function BasketZbala() {
	...
	this.start = function() {
		...
		this.hoops.push(new Hoop(120, 520), new Hoop(640 - 148 - 110, 520), new Hoop(640 / 2 - (148 / 2), 290));
	};
	
	this.drawLoadingScreen = function() {
		var ctx = this.canvas.getContext('2d');
		ctx.fillStyle = '#b1f7f7';
	...
	this.getResources = function() {
		var images = [
			...
			'res/ball.png',
			'res/hoop.png',
			'res/background-start.png',
			'res/background-over.png'
	...
	this.update = function(delta) {
		...
		if(this.state == 'play') {
					...
					for(var j = 0; j < this.hoops.length; j++) {
						...
						if(mag < 47 + 5) {
							if(!ball.scored) {
								ball.setAngle(90);
								this.score += 1;
								this.texts.push(new PopText('+ 1', hoop.x, hoop.y));
							}
							ball.scored = true;
						}
						if(!ball.scored) {
			...
			if(this.time > 0) {
				this.toNextSecond -= delta;
				if(this.toNextSecond <= 0) {
					this.time--;
					this.toNextSecond = 1;
				}
			}
	...
	this.draw = function(ctx) {
		ctx.drawImage(this.res['res/background-start.png'], 0, 0);
		if(this.state == 'menu') {
			$('#social').hide();
			//ctx.drawImage(this.res['res/title.png'], 640 / 2 - (492 / 2), 100);
			ctx.textAlign = 'center';
			//this.drawText(ctx, 'Click / Touch to Start!', 640 / 2, 520, 40);
			//this.drawText(ctx, '(?) Tap to throw ball. Try to make as many hoops before the time runs out!', 640 / 2, 940, 20);
			ctx.textAlign = 'left';
		}
		if(this.state == 'play') {
			this.zeb='social';
			...
			this.drawText(ctx, this.score, 120, 40, 40);
			this.drawText(ctx, this.time, 470, 40, 40);
			for(var i = 0; i < this.texts.length; i++) {
				var text = this.texts[i];
				text.draw(ctx);
			}
		...
		if(this.state == 'over'){
			...
			ctx.drawImage(this.res['res/background-over.png'], 0, 0);
			...
			ctx.textAlign = 'center';
			//this.drawText(ctx, 'Game Over', 640 / 2, 200, 80);
			this.drawText(ctx,this.score, 640 / 2, 230, 50);
			//this.drawText(ctx, 'Click to Continue', 640 / 2, 800, 50);
			ctx.textAlign = 'center';
		}
	};
